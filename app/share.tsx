      ...
      <ScrollView contentContainerStyle={styles.panel}>
        <ImageBackground source={require('../assets/images/nivium-hero-snow.png')} imageStyle={styles.heroImage} style={styles.hero}>
          ...
          <View style={styles.heroSubtitleStack}>
            <Text style={styles.heroSubtitle}>Share Profile</Text>
        ...
        <View style={styles.selectedShell}>
          ...
          <View style={styles.selectedPanel}>
            ...
            <Text style={styles.selectedMeta}>
              {selectedProfile
                ? `${highlights?.layerCount ?? 0} layers · ${highlights?.stabilityCount ?? 0} tests · ${selectedProfile.documentKind === 'plot' ? 'Plotted PDF' : 'Report PDF'}`
                : 'Share works from the currently selected saved profile.'}
            </Text>
              ...
              <Pressable
                ...
                onPress={() => {
                  ...
                  void (async () => {
                    ...
                    if (!renderResult.uri) {
                      Alert.alert('Share Unavailable', renderResult.error || 'The plotted renderer did not complete for this profile yet.');
                      return;
                    }
                    if (await Sharing.isAvailableAsync()) {
                      await Sharing.shareAsync(renderResult.uri);
                      return;
                    }
                    await Share.share({
                      message: selectedProfile.formattedText,
